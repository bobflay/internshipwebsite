...
class Company extends Resource
{
    ...
    public function fields(Request $request)
    {
        return [
            ...
            Image::make('Logo')->disk('public')->path('companies')->sortable()->rules('image', 'max:2048'),
            Text::make('Phone Number')->sortable()->rules('required', 'max:20'),
            Text::make('Email')->sortable()->rules('required', 'email', 'max:255'),
            Text::make('Website')->sortable()->rules('url', 'max:255'),
            Textarea::make('Description')->sortable()->rules('required', 'max:500'),
